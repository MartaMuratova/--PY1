...
class Winter_jacket(Jacket):
    ...
    def fluff_in (self, fluff):
        ...
        fluff = 1
        if fluff != 1:
            return ("Кажется в такой крутке Вы замерзнете")
        else:
            return ("Куртка будет и правда теплой")
    ...
    def set_size(self, big_size=int, size=int):
        super().set_size(size)
        if size > 10:
            size = big_size
...
class Summer_jacket(Jacket):
    ...
    def hood_on (self, hood: str):
        ...
        ans = "да"
        if hood is not ans:
            return ("Куртка без капюшона")
        else:
            return ("Куртка с капюшоном")
